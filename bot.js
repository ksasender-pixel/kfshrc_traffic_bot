...
bot.on('message', async (msg) => {
  ...
  if (session.step === 'shift_select') {
    let shiftText = '';
    if (text.includes('صباح')) shiftText = 'صباح';
    else if (text.includes('مساء')) shiftText = 'مساء';
    else if (text.includes('ليل')) shiftText = 'ليل';
    if (shiftText) {
      session.dt.shift = shiftText;
      beginReport(chatId, session);
    } else {
      bot.sendMessage(chatId, '⚠️ اختر الوردية من الأزرار أدناه');
    }
    return;
  }
...
async function handleAccident(chatId, session, text, msg) {
  ...
  if (session.step === 'acc_type') {
    ...
    d.currentParty = 1;
    d.parties = {};
    session.step = 'acc_party_vtype';
    bot.sendMessage(chatId, `🚗 الطرف الأول — نوع المركبة:`, { reply_markup: { remove_keyboard: true } });
  } else if (session.step === 'acc_party_vtype') {
    const i = d.currentParty;
    d.parties[`ط${i}_نوع_المركبة`] = text;
    session.step = 'acc_party_plate';
    bot.sendMessage(chatId, `🔢 الطرف ${i} — رقم اللوحة:`);
    ...
    if (i < d.عدد_الأطراف) {
      d.currentParty++;
      session.step = 'acc_party_vtype';
      bot.sendMessage(chatId, `🚗 الطرف ${d.currentParty} — نوع المركبة:`);
    } else {
    ...
    session.step = 'acc_tow_id';
    bot.sendMessage(chatId, `🪪 السطحة ${i} — رقم هوية السائق:`);
  } else if (session.step === 'acc_tow_id') {
    const i = d.currentTow;
    d.tows[`س${i}_الهوية`] = text;
    session.step = 'acc_tow_phone';
    bot.sendMessage(chatId, `📱 السطحة ${i} — رقم الجوال:`);
  } else if (session.step === 'acc_tow_phone') {
    const i = d.currentTow;
    d.tows[`س${i}_الجوال`] = text;
    if (i < d.عدد_السطحات) {
      d.currentTow++;
      session.step = 'acc_tow_plate';
...
async function handleService(chatId, session, text, msg) {
  ...
  if (session.step === 'svc_type') {
    ...
    d.موقع_الخدمة = text;
    session.step = 'svc_vtype';
    bot.sendMessage(chatId, '🚗 نوع المركبة:');
  } else if (session.step === 'svc_vtype') {
    d.نوع_المركبة = text;
    session.step = 'svc_plate';
    bot.sendMessage(chatId, '🔢 رقم اللوحة:');
